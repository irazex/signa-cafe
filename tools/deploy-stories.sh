...
node tools/og-images.mjs >/dev/null
ASSETS=$(node -e '
...
# Activate the new article only after every referenced image is present.
up data/stories.json
# Costs are shown through the authenticated admin endpoint, and the admin JSX
# contains the human-readable labels for the transcreation stages.
