...
- [~] **Нормальная форма входа в админку вместо Basic Auth** (задача от 05.09.2026).
      Браузерный диалог Basic Auth не показывается ни в Arc, ни во встроенном
      браузере Claude - оба идут сразу на страницу Apache «401 Unauthorized»,
      ввести пароль негде. Обход через `https://user:pass@signa.cafe/admin.html`
      документ пускает, но дальше Chromium режет собственные запросы страницы,
      и админка падает на «Could not load content.json» (сам файл публичный и
      отдаёт 200 - проверено curl). Меняем на PHP-сессию: своя страница входа
      в фирменном стиле, кука на 30 дней, кнопка выхода, троттлинг попыток.
      Порт: `admin.html` -> `admin.php`, `key.php` / `translate.php` /
      `analytics.php` / `story-costs.json` / `model-pricing.json` уходят
      с Basic Auth на проверку сессии.
