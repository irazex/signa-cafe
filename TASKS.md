...
- [~] **Выбрать постоянную модель генератора** (задача от 05.09.2026) - расчёт по
      10 моделям отдан владельцу, ждём его решения. Сейчас в дефолте `gpt-5.5`
      ($0.40/пост, $21/год), pro закрыт флагом `--allow-pro`.
      Соседняя сессия передала «владелец выбрал gpt-5.4» - в код по переданному
      решению не ставил, владелец сказал решать после расчёта лично.
